...
export class ArenaWsClient {
  ...
  /** Connect to the global stream (call once on app mount). */
  connectGlobal(): void {
  ...
  private _openGlobal(): void {
    ...
    ws.onerror = () => {
      // silently retry — global-stream events are best-effort
    }
  ...
  private _updatePhase(eventType: string, d: Record<string, unknown>): void {
    ...
    switch (eventType) {
      case 'battle.holding': {
        const down = Array.isArray(d.unhealthy) ? (d.unhealthy as string[]) : []
        set('HOLDING', down.length ? `${down.join('/')} not ready` : 'Waiting for all roles in position')
        break
      }
  ...
  private _mapAndEmit(be: BackendEvent, fromGlobal = false): void {
    // Sessions must stay isolated — one battle must never paint another's UI —
    // so ignore any event tagged with a different session_id (defensive: the
    // connection is already session-scoped).
    if (be.session_id && this.sessionId && be.session_id !== this.sessionId) {
      return
    }
    ...
    // defending, judging, reporting…). Kept here so every event
    ...
    switch (be.event_type) {
      ...
      // ── A model failed in a way that will NOT clear on its own ─────────────
      //
      // Separate from adapter.error on purpose. That one is a toast, which is right
      // for a blip and useless for an exhausted account: the run keeps retrying, the
      // toast disappears, and an unattended sweep discovers the problem hours later in
      // a log. This one sticks until the operator dismisses it and says what to do.
      case 'model.unavailable': {
        const category = (d['category'] as string) || 'unknown'
        const adviceText = (d['advice'] as string) || 'This model is not answering.'
        useGhostStore.getState().setModelFault({
          category,
          advice: adviceText,
          detail: ((d['detail'] as string) || '').slice(0, 400),
          round: (d['round'] as number) ?? 0,
          at: Date.now(),
        })
        useGhostStore.getState().pushLog({
          agentId: 'system',
          message: `MODEL ${category.replace(/_/g, ' ').toUpperCase()} — ${adviceText}`,
          state: 'failed',
        })
        this.emit({ type: 'system_alert', message: `⚠ ${adviceText}` })
        break
      }
