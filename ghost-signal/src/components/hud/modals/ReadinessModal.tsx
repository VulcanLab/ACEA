...
type Data = {
  ...
  blue: string
  rounds: number
  onConfirm?: (opts?: { inner: boolean; outer: boolean }) => void
}
...
export default function ReadinessModal() {
  ...
  // Per-battle improvement loops — opt-in, default off.
  const [innerLoop, setInnerLoop] = useState(false)
  const [outerLoop, setOuterLoop] = useState(false)
    ...
    <div className="modal-backdrop" onClick={closeModal}>
      ...
      >
        ...
        <div className="px-4 py-3 overflow-y-auto" style={{ minHeight: 200 }}>
          ...
          {/* Evolution loops — optional per-battle self-improvement (default off) */}
          <div className="mt-3 pt-2.5" style={{ borderTop: '1px solid #1e293b' }}>
            ...
            <label className="flex items-start gap-2 text-[10px] font-mono cursor-pointer" style={{ color: '#cbd5e1' }}>
              <input type="checkbox" checked={outerLoop} onChange={e => setOuterLoop(e.target.checked)} style={{ accentColor: OK, marginTop: 2 }} />
              <span>Outer loop — code-level self-improvement: rebuild + benchmark + promote/rollback</span>
            </label>
          </div>
        </div>
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              disabled={!canLaunch}
              onClick={() => { data?.onConfirm?.({ inner: innerLoop, outer: outerLoop }); closeModal() }}
