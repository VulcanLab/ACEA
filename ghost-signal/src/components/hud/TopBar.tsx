...
export default function TopBar() {
  ...
  const doLaunch = async (roundsArg: number, inner = false, outer = false) => {
    setError('')
    setLaunching(true)
    // Record the per-battle loop selection so the scene can gate assist/ASIS
    // visuals to what actually runs.
    useGhostStore.getState().setLoopFlags(inner, outer)
    // Fresh thinking-chat per battle; remember config for RUN AGAIN.
    useGhostStore.getState().clearAgentChat()
    useGhostStore.getState().setLastLaunch({ red: redId, blue: blueId, rounds: roundsArg, inner, outer })
    try {
      const res = await arenaApi.startBattle(redId, blueId, roundsArg, 'deathmatch', inner, outer)
  ...
  const startLive = () => {
    ...
    useGhostStore.getState().openModal('battle_readiness', {
      ...
      onConfirm: (opts?: { inner: boolean; outer: boolean }) => {
        void doLaunch(roundsArg, opts?.inner ?? false, opts?.outer ?? false)
      },
    })
  }
  ...
  const runAgain = () => {
    ...
    g.openModal('battle_readiness', {
      red, blue, rounds,
      onConfirm: (opts?: { inner: boolean; outer: boolean }) =>
        void doLaunch(rounds, opts?.inner ?? last?.inner ?? false, opts?.outer ?? last?.outer ?? false),
    })
  }
