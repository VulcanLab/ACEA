...
Copy `adapters/red-template/adapter.py` or `adapters/blue-template/adapter.py`
from this repository into your project root.
Fill in the `generate_attack_payload` / `evaluate_defense` logic with your own code.
...
> `decision` must be one of: `"block"` | `"allow"` | `"modify"`  
> If `"modify"`, set `rewritten_payload` to the sanitized input.
