...
def default_scenario(inventory=None, objective_kinds=None):
    """The engagement the platform has always run, now stated explicitly.

    `inventory` is what the target publishes about the material it holds: each value
    with the kind of thing it is. `objective_kinds` is which of those kinds serve THIS
    battle's declared objective, resolved per battle from the participants' own
    declarations — see `resolve_objective_basis`. The judge holds no view of its own
    about what an attacker is after: a project whose purpose is the system prompt and
    one whose purpose is customer data are scored on different material because they
    declared different aims, not because the platform sorted them.

    Passing no kinds means the basis was not narrowed, and every published item counts —
    the behaviour before any of this, stated rather than implied. Without an inventory
    the markers come from the same configuration that previously drove the hardcoded
    rule, so an operator who set that keeps exactly what they had.
    """
    kinds = {str(k).strip() for k in (objective_kinds or []) if str(k).strip()}
    markers = []
    for item in (inventory or []):
        if not isinstance(item, dict):
            continue
        if kinds and str(item.get("kind", "")) not in kinds:
            continue
...
def load(spec=None, *, scenarios_dir=None, inventory=None, objective_kinds=None):
    ...
    can add an engagement without touching code. `inventory` and `objective_kinds` only
    affect the default engagement: a declared engagement already states its own proof,
    and is taken at its word.
    """
    if spec is None or spec == "":
        return default_scenario(inventory, objective_kinds)
