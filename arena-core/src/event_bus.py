...
# Events that are also written to the global fan-in stream for cross-session consumers
...
async def publish_event(session_id: str, event_type: str, data: dict[str, Any]) -> None:
    ...
    # Fan-in: write select events to global stream for cross-session consumers
...
async def read_global_events(last_id: str = "0") -> list[dict]:
    """Read from the global fan-in stream (cross-session events: battle.complete)."""
    r = await get_redis()
