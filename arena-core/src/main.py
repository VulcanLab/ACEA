...
async def _run_migrations() -> None:
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            "ALTER TABLE battle_sessions "
            "ADD COLUMN IF NOT EXISTS outer_loop_enabled BOOLEAN DEFAULT FALSE"
        )
...
async def agents_roster():
    """The platform's self-improvement AI roster — the assisting agents that
    ...
    in-battle context injection). Plus a shared ASIS Meta-Agent (code-improver,
    post-battle code rewriting). Drives the frontend sprite labels/count so the
    UI reflects how many AIs actually collaborate.
    """
    ...
    return {
        "red": [
            ...
            role("atk3", "recon", "Recon Analyst",
                 settings.red_recon_model or settings.meta_agent_model or settings.analyzer_model),
        ],
        "blue": [
            ...
            role("def3", "recon", "Recon Analyst",
                 settings.blue_recon_model or settings.meta_agent_model or settings.analyzer_model),
        ],
        "shared": [
            role("asis", "meta_agent", "ASIS Meta-Agent", settings.meta_agent_model),
        ],
    }
...
async def _readiness_for_side(role: str, service_id: str) -> dict:
    """Light per-side admission for the pre-flight readiness panel.
    ...
    if health == "ok":
        if role == "red":
            if not (caps.get("supports_attack_generation", True) or caps.get("attack_type")):
                reasons.append("wrong_role_capabilities")
        else:
            if not (caps.get("supports_input_guard", False) or caps.get("supports_output_guard", False)):
                reasons.append("wrong_role_capabilities")
...
async def battle_readiness(red: str = "", blue: str = ""):
    ...
    # ASIS may be briefly rebuilding a side's adapter container (self-improvement).
    # Surface that as a clear, transient blocker instead of a confusing health error.
    rebuilding = {"red": False, "blue": False}
    try:
        from event_bus import get_redis
        r = await get_redis()
        for side in ("red", "blue"):
            rebuilding[side] = bool(await r.get(f"asis:rebuilding:{side}"))
    except Exception:
        pass
    ...
    for side in ("red", "blue"):
        if rebuilding[side]:
            blockers.append({"scope": side, "code": "adapter_improving",
                "message": f"The {side} adapter is being improved by ASIS (rebuilding). Re-check in ~1 minute."})
    ...
    if not models_ok:
        blockers.append({"scope": "models", "code": "models_unreachable",
            "message": "One or more configured models are unreachable. Open Model Health and fix your .env model names."})
