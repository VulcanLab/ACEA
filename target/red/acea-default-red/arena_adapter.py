...
def _pick_objective(target_purpose: str, generation: int, session_objective: str = "") -> str:
    """What to aim at this round.

    Order matters. A per-battle objective arrives in the request and describes the
    engagement actually being scored, so it wins: without this the adapter pursued a
    deployment-wide goal from its own environment while the platform graded something
    else, and no declared engagement could change what was attempted. The environment
    default is next, and the purpose-matched list is the fallback for a deployment that
    declares nothing at all.
    """
    if session_objective.strip():
        return session_objective.strip()
...
async def generate_attack(req: AttackRequest):
    ...
    # The engagement, as the orchestrator described it this round. Both are absent
    # for a plain battle, which then behaves exactly as it always did.
    meta = req.metadata or {}
    session_objective = str(meta.get("red_team_objective") or "")
    target_actions = [a for a in (meta.get("arena_target_actions") or []) if isinstance(a, dict)]

    # Learn from the judge's feedback: boost suggested technique, decay avoided.
    # This is what makes the default red converge over rounds.
    # Recorded against this engagement's shape — what works for disclosure is not
    # evidence about what makes a target act outside its authority.
    strategies.record_feedback(suggested=mutation, avoid=avoid,
                               actions_offered=bool(target_actions))

    technique, context, pressure = strategies.pick_layers(
        mutation_hint=mutation, avoid=avoid,
        actions_offered=bool(target_actions),
    )
    objective = _pick_objective(target_purpose, generation, session_objective)
