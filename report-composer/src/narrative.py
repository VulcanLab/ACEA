...
async def _call_llm(prompt: str) -> dict[str, Any]:
    ...
    try:
        ...
        resp = await litellm.acompletion(
            ...
            temperature=0.3,
            # The full report has ~10 sections incl. per-round causal analysis;
            # 6000 truncated mid-report (Blue section onward got cut). Give headroom.
            max_tokens=14000,
...
def _fallback_narrative(
    ...
    reason = "LLM narrative unavailable (call failed or misconfigured)." if llm_used else (
        "Narrative LLM disabled — set LITELLM_BASE_URL for prose report."
    )
    ...
    for r in rounds:
        ...
        detail_blocks.append(
            ...
            f"- **Outcome**: {'RED WIN' if r.get('red_success') else 'BLUE WIN'}\n"
        )
...
async def _fetch_generation_history(
    pool: asyncpg.Pool,
    red_service_id: str,
    blue_service_id: str,
) -> dict[str, list[dict]]:
    """Fetch ASIS adapter generation history for both teams."""
    result: dict[str, list[dict]] = {"red": [], "blue": []}
    async with pool.acquire() as conn:
        for team, sid in [("red", red_service_id), ("blue", blue_service_id)]:
            if not sid:
                continue
            rows = await conn.fetch(
                "SELECT gen_number, benchmark_asr, benchmark_dr, is_active, "
                "rollback_reason, patch_diff, created_at FROM adapter_generations "
                "WHERE adapter_id=$1 ORDER BY gen_number ASC",
                sid,
            )
            result[team] = [dict(r) for r in rows]
    return result


_MILESTONE_CAP = 8  # max accepted-generation rows shown before we summarize the rest


def _gen_status(g: dict[str, Any]) -> str:
    if g.get("is_active"):
        return "active"
    if g.get("rollback_reason"):
        return "rolled back"
    return "superseded"


def _format_learning_curve(gen_history: dict[str, list[dict]]) -> str:
    """Compact ASIS generation summary.

    A long-running session can produce hundreds of generations, most of which
    are rolled-back experiments. Dumping every row is unreadable, so we surface
    only the MILESTONES — generations that were accepted into the active line
    (active or later superseded) — plus a one-line tally of the rolled-back
    attempts that were tried and discarded. Only the ACTIVE diff is shown.
    """
    red = gen_history.get("red", [])
    blue = gen_history.get("blue", [])
    if not red and not blue:
        return ""

    lines = ["\n## ASIS Generation Learning Curve\n"]
    lines.append(
        "_Milestones only. Each accepted generation is a real, benchmarked code rewrite; "
        "rolled-back rows are failed experiments the platform discarded automatically._\n"
    )

    for team_name, gens in [("Red", red), ("Blue", blue)]:
        if not gens:
            lines.append(f"### {team_name} Team\nNo ASIS improvements applied.\n")
            continue

        total = len(gens)
        rolled = sum(1 for g in gens if _gen_status(g) == "rolled back")
        superseded = sum(1 for g in gens if _gen_status(g) == "superseded")
        promoted = sum(1 for g in gens if g.get("is_active"))

        lines.append(f"### {team_name} Team\n")
        lines.append(
            f"**{total} generations** · {promoted} active · {superseded} superseded · "
            f"{rolled} rolled back (discarded experiments).\n"
        )

        # Milestones = accepted generations (active/superseded) that actually have
        # benchmark numbers — skip rows with no data (they render as empty "—" noise),
        # but always keep the active generation even if its benchmark is missing.
        milestones = [
            g for g in gens
            if _gen_status(g) in ("active", "superseded")
            and (g.get("is_active")
                 or g.get("benchmark_asr") is not None
                 or g.get("benchmark_dr") is not None)
        ]
        shown = milestones[-_MILESTONE_CAP:] if len(milestones) > _MILESTONE_CAP else milestones
        omitted = len(milestones) - len(shown)

        if shown:
            lines.append("| Gen | Bench ASR | Bench DR | Status |")
            lines.append("|-----|-----------|----------|--------|")
            if omitted > 0:
                lines.append(f"| … | | | _{omitted} earlier milestone(s) omitted_ |")
            for g in shown:
                asr_s = f"{g['benchmark_asr']*100:.1f}%" if g.get("benchmark_asr") is not None else "—"
                dr_s  = f"{g['benchmark_dr']*100:.1f}%"  if g.get("benchmark_dr")  is not None else "—"
                lines.append(f"| gen_{g['gen_number']} | {asr_s} | {dr_s} | {_gen_status(g)} |")
            lines.append("")

        # Show only the most recent PROMOTED (active) diff — the change actually kept.
        active = [g for g in gens if g.get("is_active") and g.get("patch_diff")]
        if active:
            latest = active[-1]
            diff = (latest.get("patch_diff") or "").strip()
            if diff:
                if len(diff) > 4000:
                    diff = diff[:4000] + "\n... (diff truncated)"
                # Blank lines around the fence guarantee python-markdown opens a
                # fenced code block (otherwise `-` diff lines degrade into a bullet list).
                lines.append(f"#### {team_name} — code kept in gen_{latest['gen_number']} (active)")
                lines.append("")
                lines.append("```diff")
                lines.append(diff)
                lines.append("```")
...
async def generate_narrative(
    ...
    if not llm_result or "zone_insights" not in llm_result:
        llm_result = _fallback_narrative(session, stats, rounds, llm_used=bool(settings.litellm_base_url))
    ...
    # ── ASIS: append generation learning curve ────────────────────────────
    red_svc  = session.get("red_service_id",  "")
    blue_svc = session.get("blue_service_id", "")
    if red_svc or blue_svc:
        try:
            gen_hist = await _fetch_generation_history(pool, red_svc or "", blue_svc or "")
            curve = _format_learning_curve(gen_hist)
            if curve:
                # A blank line MUST separate the LLM narrative from the appended
                # section — otherwise, when the narrative ends with a markdown
                # table (no trailing blank line), the "## ASIS…" heading merges
                # into that block and renders as literal text.
                narrative = (narrative or "").rstrip() + "\n\n" + curve.lstrip("\n")
        except Exception as exc:
            log.warning("ASIS learning curve fetch failed: %s", exc)
