...
COMPREHEND = os.getenv("COMPREHENSION_URL", "http://localhost:8010")
...
async def t_blue_output_guard(client: httpx.AsyncClient) -> bool:
    ...
    if not d1.get("was_modified") or "123-45-6789" in d1.get("final_response", ""):
        ...
        # gap in the plugged-in blue project (no PII regex), not a platform failure.
        warn("blue output-guard PII",
             f"external blue did not redact SSN (capability gap): "
...
async def main() -> int:
    ...
    async with httpx.AsyncClient() as client:
        ...
        await asyncio.gather(
            ...
            t_health(client, "comprehension",  COMPREHEND, "comprehension"),
